...
// Animation when closing youtubeplayeractivity (none for default)
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    ...
    content_desc.setBackgroundColor(Color.parseColor(getString(R.color.background_color)));
    content_desc.setFocusableInTouchMode(false);
    content_desc.setFocusable(false);
    content_desc.getSettings().setDefaultTextEncodingName("UTF-8");

    String mimeType = "text/html;charset=UTF-8";
    String encoding = "utf-8";
    String htmlText = news.getDescription();
    ...
    content_desc.loadData(text, mimeType, encoding);
//      content_desc.setVerticalScrollBarEnabled(false);
//      content_desc.setScroller(null);
   if (!news.getNews_image().equals("")) {
      Picasso.with(this.getActivity())
              .load(Constant.SERVER_IMAGE_NEWSLISTDETAILS+news.getNews_image())
              .placeholder(R.drawable.placeholder_normal)
              .error(R.drawable.placeholder_normal) // optional
              //   .resize(0, 250)
              .into(imageNews);
    }
    //Toast.makeText(getActivity(),""+Constant.SERVER_IMAGE_NEWSLISTDETAILS+news.getNews_image(),Toast.LENGTH_LONG).show();
    //imageLoader.DisplayImage(Constant.SERVER_IMAGE_NEWSLISTDETAILS+news.getNews_image(), imageNews);
